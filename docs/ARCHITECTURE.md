...
iPhone / Web / Shortcuts / NFC
   ...
   ├── Server Components (páginas)     → leen Postgres vía Prisma directamente
   ...
   ┌──────────┼───────────────┐
   │          │               │
Postgres   GitHub API      Cloudflare R2
...
| PostgreSQL + Prisma | Supabase, Mongo, SQLite | Relacional encaja con el modelo (proyectos/items/relaciones); Prisma da migraciones tipadas sin acoplarse a un proveedor |
...
| Docker Compose | Kubernetes, PaaS gestionado | Un servidor (Hostinger) alcanza; Docker da reproducibilidad sin orquestación innecesaria |
